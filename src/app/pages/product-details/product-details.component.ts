...
export class ProductDetailsComponent implements OnInit {
  ...
  submitRating(rating: number): void {
    // Ensure that rating is a valid number between 1 and 5
    ...
    this.productService.updateProductRating(productId, rating).subscribe({
      next: (response) => {
        ...
        // Update the product rating and count in the UI
        this.product.ratingAverage = response.data.ratingAverage;
        this.product.ratingCount = response.data.ratingCount;
      },
      error: (err) => {
